...
namespace Hazel
{
	Ref<VertexBuffer> VertexBuffer::Creat(uint32_t size)
	{
		switch (Render::GetAPI())
		{
		case RenderAPI::API::None: HZ_CORE_ASSERT(false, "RenderAPI::None is currently not supported. "); return nullptr;
		case RenderAPI::API::OpenGL: return make_Ref<OpenGLVertexBuffer>(size);
		}
	...
	Ref<IndexBuffer> IndexBuffer::Creat(uint32_t count)
	{
		switch (Render::GetAPI())
		{
		case RenderAPI::API::None: HZ_CORE_ASSERT(false, "RenderAPI::None is currently not supported. "); return nullptr;
		case RenderAPI::API::OpenGL: return make_Ref<OpenGLIndexBuffer>(count);
		}
