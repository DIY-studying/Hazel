...
void main()
{
	...
	// 环境光
	vec3 amb_color= u_light_ka*u_light_amb_instensity*vec3(tex_color);
	...
	float cos_angle=max(dot(nor,light_dir),0);
	vec3 diffuse_color=u_light_kd*u_light_Instensity/r_square*cos_angle*vec3(tex_color);


	// 高光
	vec3 view_dir=normalize(u_viewPos-v_Position);
	vec3 nor_dir=normalize(light_dir+view_dir);
	float cos_spe=pow(dot(nor_dir,nor),64);
